...
def about_html() -> str:
    ...
    return (
        ...
        f"<p>{APP_NAME} is dual licensed: the model under GPL-3.0 and the user "
        "interface under LGPL-3.0. See the Help menu for both licences.</p>"
        f"<p>{APP_NAME} reads your music folder and never writes to it. Every "
        "piece of state it keeps lives in its own store: your ratings, your "
        "play counts, the artwork it finds and every correction you accept in "
        "the repair screen. Repairing the library changes that store and never "
        "a music file, so a correction can always be taken straight back out "
        ...
        f"<p>{APP_NAME} asks these public services when you go looking for "
        "music you do not hold. It asks them again for a cover it cannot "
