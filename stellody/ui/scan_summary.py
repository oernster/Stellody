...
def _totals(change: LibraryChange, report: ScanReport) -> str:
    """The library as it now stands, plus what the scan had to do to say so."""
    rows = [
        ("Albums", str(change.total_albums)),
        ("Tracks", str(change.total_tracks)),
        ("Folders read", str(report.folders_probed)),
        ("Folders unchanged", str(report.folders_reused)),
        ("Files read", str(report.files_probed)),
    ]
    if report.files_unreadable:
        rows.append(("Files that could not be read", str(report.files_unreadable)))
    if report.files_absent:
        rows.append(("Files no longer there", str(report.files_absent)))
    cells = "".join(
        f"<tr><td>{name}</td><td align='right'><b>{value}</b></td></tr>"
        for name, value in rows
    )
    return f"<h3>Your library now</h3><table width='100%'>{cells}</table>"
