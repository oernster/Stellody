...
opens with its controls past the edge. It is clamped to what the window says
it needs too, since a stored size smaller than that is one nobody can use. A value
that is not a number at all falls back to the size a first run opens at, which
is the same fallback the volume and the cover size already use.
"""
