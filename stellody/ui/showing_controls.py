"""The two controls saying what the library is shown as.

They sit together on the bottom strip because both change what is on show
rather than what is playing: what the library is drawn as and how large the
sleeves are. The equalizer used to follow them; it shapes what is heard rather
than what is shown, so it now sits with the volume in `sound_controls.py`.
...
class ShowingControls(QWidget):
    ...
    def stops(self) -> tuple[QPushButton, ...]:
        ...
        return (self.view_button, self.size_button)
