...
class LibraryWalker(Protocol):
    ...
    def reachable(self, root: str) -> bool:
        """Whether the root is there to be walked at all.

        Asked because a walk cannot tell the difference by itself. Measured on
        2026-09-10: a root on a drive that is not connected walks as no
        folders with no error, exactly as an empty library does.
        """
        ...
