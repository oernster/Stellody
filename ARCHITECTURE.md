...
| Every block the device ran dry before it arrived is written to the diary | Reported by Oliver on 2026-09-16: a few seconds of static during playback while a Nuitka build of another project held all 24 cores, never permanent. The cause is not known yet; the hypothesis is a feeder thread starved of time, so the device empties before the next block. PortAudio answers underflowed from a write that found the device already dry (measured in the installed sounddevice source); `stellody/infrastructure/dropouts.py` notes each one with a running count and the milliseconds since the write before: a long gap points at a starved feeder, a gap no longer than a block takes to play points elsewhere. `play` forgets the last write, so a pause is never read as a late one. The visualiser's measuring moved to `stellody/infrastructure/metering.py` in the same change to keep `audio.py` out of the danger band. `tests/infrastructure/test_dropouts.py` holds the wording and drives the shipped feeder with a stream that reports every write dry, proved by planting a feeder that ignores the answer. No fix is made until the diary shows the dropouts lining up with the static. |
