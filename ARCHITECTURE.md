...
| A pause is not an ending; only the transport can tell | Reported against a real library: pausing a track then pressing play started it from its beginning. The device reports the two the same way, because the feeder clears the same flag whether a track ran out or a listener held it, so `advance_if_finished` read a pause as an ending at the next poll a quarter of a second later. On the last track of a queue that gave the device back, which is what made the next press reload from nothing rather than resume; in the middle of one it moved silently to the next track while the listener sat on this one. Which of the two happened is knowable only where the press was received, so the transport remembers it: `_held` is set when a listener pauses, cleared when they resume and set from `playing` at every load, since a track opened without playing is one somebody is sitting on. Proved by planting the removal: the four hold cases fail and both ending cases still pass. |
