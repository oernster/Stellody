...
| A three-state switch says its state with a picture, not with a fill | The bottom strip shows a setting by lighting the button behind one unchanging picture, which can say exactly two things. Repeat now holds three, so each state carries its own artwork and the fill is left saying only that something is on. Shuffle keeps the two-state rule, since it has two states. |
