...
| 10 | A ring belongs to a control; to every control. No container is named as a ring target, no item view wears one in any state, no pane reaches the window's focus chain; every control that Tab can land on names a ring, walked off the real widgets rather than off a list. | `tests/ui/test_focus_rings.py` |
