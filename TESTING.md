...
enforces it; `DISCOVERY.md`, `SHOPS.md` and `FORMATS.md` name the test behind
every requirement. This file is how to work with those tests, not a second copy
of what they assert.
...
- **One `QApplication`, no window outliving its test.** `tests/ui/conftest.py`
  ...
  widget between tests; `tests/infrastructure/conftest.py` provides the same
  fixture to the suites there, so no suite builds its own. A window left to the garbage collector was destroyed
