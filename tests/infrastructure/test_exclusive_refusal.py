...
def test_a_lossy_source_is_refused_exclusive_for_the_file(
    ...
    def mixer(_device, _request, reason: str) -> None:
        handed.append(reason)

    monkeypatch.setattr(wasapi, "_shared_result", mixer)
    ...
    assert handed == [wasapi.NO_STATED_DEPTH]
    assert "file" in wasapi.NO_STATED_DEPTH, "the file is named, not the device"
